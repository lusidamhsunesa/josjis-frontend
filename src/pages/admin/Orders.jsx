...
const OrderCard = ({ order, onUpdateStatus, onDetail }) => {
  const getStatusInfo = (status) => {
    switch (status) {
      case 'Menunggu': return { label: 'Baru', color: 'bg-[#ffd900]', textColor: 'text-[#743b0e]' };
      case 'Diproses': return { label: 'Diproses', color: 'bg-blue-500', textColor: 'text-white' };
      case 'Selesai': return { label: 'Selesai', color: 'bg-green-500', textColor: 'text-white' };
      default: return { label: status, color: 'bg-gray-400', textColor: 'text-white' };
    }
  };
  ...
  return (
    <div className="bg-white border border-black rounded-[15px] p-[20px] flex flex-col min-h-[323px] shadow-lg transition-transform hover:scale-[1.02]">
      ...
      <div className="flex justify-between items-start mb-2">
        <div className={`${statusInfo.color} px-[12px] py-[2px] rounded-[10px] shadow-sm`}>
           <span className={`font-inter font-bold ${statusInfo.textColor} text-[12px]`}>{statusInfo.label}</span>
        </div>
        <div className="text-right">
           <p className="font-inter font-bold text-[#5f5f60] text-[16px] truncate max-w-[150px]">{order.customer}</p>
           <p className="font-inter text-[#bbbec9] text-[11px] uppercase">Order {order.id}</p>
        </div>
      </div>
        ...
        {order.items_list?.map((item, idx) => (
          <div key={idx} className="flex justify-between items-start">
            <div className="flex gap-2">
              <span className="font-inter font-bold text-[12px] text-black shrink-0">{item.quantity}x</span>
              <span className="font-inter font-bold text-[12px] text-black line-clamp-1">{item.name}</span>
            </div>
            <span className="font-inter text-[11px] text-black font-medium shrink-0">{item.price}</span>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-100">
        <div>
          ...
          <p className="font-inter font-bold text-[#636364] text-[16px]">{order.table}</p>
        </div>
        <div>
          <p className="font-inter text-[#b8bbc6] text-[11px]">Total Pembayaran</p>
          <p className="font-inter font-bold text-[#5e5e5f] text-[16px]">{order.total}</p>
        </div>
      </div>
      ...
      {order.notes && (
        <div className="mt-3">
          ...
          <p className="font-inter text-[#747576] text-[12px] italic line-clamp-2 leading-tight">{order.notes}</p>
        </div>
      )}
      ...
      <div className="flex gap-3 mt-6">
        {order.status === 'Menunggu' && (
          <button 
            onClick={(e) => { e.stopPropagation(); onUpdateStatus(order.id, 'Diproses'); }}
          ...
          >
            ...
            <span className="font-roboto font-medium text-[#ffd900] text-[13px]">Terima</span>
          </button>
        )}
        {order.status === 'Diproses' && (
          <button 
            onClick={(e) => { e.stopPropagation(); onUpdateStatus(order.id, 'Selesai'); }}
          ...
          >
            <img src={imgVector7} alt="" className="size-[20px]" />
            <span className="font-roboto font-medium text-white text-[13px]">Selesai</span>
          </button>
        )}
        <button 
          onClick={() => onDetail(order.id)}
          className={`bg-[#ffd900] h-[40px] rounded-[13px] flex items-center justify-center gap-2 hover:bg-[#e6c400] transition-colors ${order.status === 'Selesai' ? 'w-full' : 'flex-[1.5]'}`}
        >
          <span className="font-roboto font-medium text-black text-[13px]">Detail Pesanan</span>
      ...
      <div className="p-[20px] md:p-[37px] pb-[100px]">
         ...
         <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-[35px]">
          <div className="w-full sm:w-[366px] min-h-[122px] bg-gradient-to-r from-[#d20102] to-[#770001] rounded-[15px] border border-black p-[24px] pl-[29px] flex flex-col justify-center shadow-lg">
            <h2 className="font-roboto font-extrabold text-[28px] text-white">Pesanan</h2>
            <p className="font-roboto text-[18px] text-white mt-[5px]">Daftar semua pesanan</p>
          </div>

          <div className="relative mt-[5px] sm:mt-[15px] self-end sm:self-auto cursor-pointer" onClick={() => window.location.href = '/admin/orders'}>
        ...
        <div className="flex gap-4 md:gap-[32px] mb-[40px] overflow-x-auto pb-4 scrollbar-hide">
           {[
             ...
             <div key={i} className="min-w-[180px] md:min-w-[210px] flex-1 h-[109px] bg-[rgba(217,217,217,0.5)] border border-white rounded-[15px] shadow-lg p-[19px] flex flex-col justify-between backdrop-blur-md">
                <p className="font-roboto font-semibold text-[18px] md:text-[20px] text-white whitespace-nowrap">{stat.label}</p>
                <p className="font-roboto font-extrabold text-[22px] md:text-[25px] text-white leading-none">{stat.val}</p>
             </div>
           ))}
        </div>
        ...
        <div className="space-y-[25px]">
           <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div className="bg-gradient-to-r from-[#d20102] to-[#770001] h-[72px] w-full xl:w-[500px] flex items-center px-[29px] rounded-[15px] border border-black shadow-md">
                 <h3 className="font-roboto font-semibold text-[20px] text-white uppercase tracking-wider">Pesanan Masuk</h3>
              </div>
              ...
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => (
                  <OrderCard 
                    ...
                    onDetail={(id) => navigate(`/admin/order/${id.replace('#', '')}`)}
                  />
                ))
              ) : (
                <div className="col-span-full py-[100px] text-center bg-white/10 rounded-[15px] border border-white/20 backdrop-blur-sm shadow-inner">
                   <p className="font-roboto text-white text-[20px] opacity-70">Tidak ada pesanan ditemukan.</p>
                </div>
              )}
           </div>
        </div>

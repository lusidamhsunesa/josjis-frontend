...
const INITIAL_REVIEWS = [
  { name: 'Alex Trie', rating: 5, comment: 'Penyetan ayamnya enak banget, sambalnya pedas mantap! Service juga cepat.' },
  { name: 'Budi Santoso', rating: 4, comment: 'Lele gorengnya garing, nasi hangat. Cuma sayang tadi nunggunya agak lama pas ramai.' },
  { name: 'Siti Aminah', rating: 5, comment: 'Langganan terus di sini, harga terjangkau rasa bintang lima!' },
  { name: 'Dedi Kurniawan', rating: 3, comment: 'Rasa oke, tapi es tehnya kurang manis tadi.' },
];
      ...
      <div className="p-[37px]">
         ...
         <div className="flex justify-between items-start mb-[33px]">
          <div className="relative w-[409px] h-[122px] bg-gradient-to-r from-[#d20102] to-[#770001] rounded-[15px] border border-black p-[22px] pl-[23px]">
            <h2 className="font-roboto font-extrabold text-[28px] text-white">Rating & Review</h2>
            <p className="font-roboto text-[18px] text-white mt-[5px]">Bintang rata-rata: {averageRating} / 5.0</p>
          </div>
        ...
        <div className="bg-[rgba(217,217,217,0.5)] border border-white rounded-[15px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25),0px_10px_12px_0px_rgba(0,0,0,0.5)] p-[30px] min-h-[800px] space-y-[20px]">
           {reviews.map((review, i) => (
             <div key={i} className="bg-white/80 border border-black/10 rounded-[10px] p-[20px] shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-center mb-[10px]">
                   <h4 className="font-roboto font-bold text-[18px] text-black">{review.name}</h4>
                   <div className="flex">
                      {[...Array(5)].map((_, star) => (
                        <div key={star} className="size-[20px] relative flex items-center justify-center">
                           <img 
                            alt="star" 
                            src={imgVector} 
                ...
                <p className="font-roboto text-[16px] text-black/80 italic">"{review.comment}"</p>
             </div>
           ))}
        </div>
      </div>
